...
namespace ZYNet.CloudSystem.Frame //NOT USE System.Reflection
{
    ...
    internal static class DispatchProxyGenerator
    {
        ...
        private static Type GenerateProxyType(Type baseType, Type interfaceType)
        {
            ...
            if (!interfaceType.GetTypeInfo().IsInterface)
            {
                ...
                throw new ArgumentException(string.Format("InterfaceType Must Be Interface {0}", interfaceType.FullName), "T");
            }
            ...
            if (baseTypeInfo.IsSealed)
            {
                ...
                throw new ArgumentException(string.Format("SR BaseType Cannot Be Sealed", baseTypeInfo.FullName), "TProxy");
            }

            // The base type cannot be abstract
            if (baseTypeInfo.IsAbstract)
            {
                throw new ArgumentException(string.Format("BaseType Cannot Be Abstract", baseType.FullName), "TProxy");
            ...
            {
                throw new ArgumentException(string.Format("BaseType Must Have Default Ctor", baseType.FullName), "TProxy");
